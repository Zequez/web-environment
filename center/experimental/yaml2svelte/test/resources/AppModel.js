...
var root = $.template(`<div class="flex flex-col min-h-screen relative"><div class="bg-gray-100 text-gray-950 dark:(bg-gray-950! text-gray-100!)"><!></div> <div id="nav-start"></div> <div><span class="absolute inset-0 pointer-events-none b-t-1 b-b-1 b-black/20 dark:b-white/50"></span> <a href="#nav-start" class="font-serif relative"><div class="absolute top-10% right-100% mr1 h-80% opacity-20 dark:filter-invert"><img class="h-full max-w-none" alt="Intricate design on left of name"></div> Ezequiel Adrián Schwartzman <div class="absolute top-10% left-100% ml1 h-80% scale-x-[-1] opacity-20 dark:filter-invert"><img class="h-full max-w-none" alt="Intricate design on right of name"></div></a></div> <div class="relative flex-grow bg-gray-950 text-white"><!></div> <!></div>`);
...
export default function _unknown_($$anchor, $$props) {
	...
	$.set_attribute(img_1, 'src', guarda);
	$.reset(div_4);
	$.reset(a);
	$.reset(div_2);
	...
	$.set_style(div_5, `${`background-image: url(${noise});`}}`);
